...
import { EXAMENES_PREVISTOS } from "../data/mockData";
...
export function ExamenesPrevistosScreen() {
  const { t } = useI18n();
  ...
  return (
    <ScreenLayout
      headerMode="back"
      backTitle={t("exams.upcoming.title")}
      helpKey="examenes-previstos"
    >
            ...
            {examenesFiltrados.map((exam) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  {!exam.confirmado && (
                    <span className="flex items-center gap-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                      {t("exams.registrationOpen")}
                    </span>
                  )}
                </div>
                ...
                <button
                  onClick={() =>
                    alert(t("exams.enrolledIn").replace("{title}", exam.title))
                  }
