...
import { EXAMENES_REALIZADOS } from "../data/mockData";
...
function matchesFilter(sortDate, filterValue) {
  const daysAgo = getDaysFromNow(sortDate);
...
export function ExamenesRealizadosScreen() {
  const { t } = useI18n();
  const FILTER_OPTIONS = [
    { value: "7d", label: t("exams.completed.last7") },
    { value: "1m", label: t("exams.completed.lastMonth") },
    { value: "6m", label: t("exams.completed.last6") },
    { value: "all", label: t("exams.upcoming.over6") },
  ];
  const formatFilterLabel = (value) => {
    const opt = FILTER_OPTIONS.find((o) => o.value === value);
    return opt ? opt.label : t("exams.completed.last7");
  };
  const [searchQuery, setSearchQuery] = useState("");
  const [filterValue, setFilterValue] = useState("6m");
  ...
  const examenesFiltrados = useMemo(() => {
    let result = [...EXAMENES_REALIZADOS];
  ...
  return (
    <ScreenLayout
      headerMode="back"
      backTitle={t("exams.completed.title")}
      helpKey="examenes-realizados"
    >
            ...
            {examenesFiltrados.map((exam) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  {exam.resultado === "APTO" ? (
                    <span className="flex items-center gap-1 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                      <Check size={12} /> {t("exams.passed")}
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                      <X size={12} /> {t("exams.failed")}
                    </span>
                  )}
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 text-xs pt-1 border-t border-gray-100 dark:border-gray-700">
                    <span className="text-gray-500 dark:text-gray-400 w-20 flex-shrink-0">
                      {t("exams.grade")}
                    </span>
                    <span
                      className={`font-bold text-sm ${exam.resultado === "APTO" ? "text-green-600" : "text-red-600"}`}
                    >
                      {exam.nota}
                    </span>
                  </div>
